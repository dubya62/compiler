...
def handle_generalization(toks:Tokens):
    ...
    toks.combine_all(["blockIdx", ".", "x"])
    toks.combine_all(["blockIdx", ".", "y"])
    toks.combine_all(["blockDim", ".", "x"])
    toks.combine_all(["blockDim", ".", "y"])
    toks.combine_all(["threadIdx", ".", "x"])
    toks.combine_all(["threadIdx", ".", "y"])
    ...
    while i < n:
        if toks[i] == "{":
            ...
            if is_defined(toks[i]) or toks[i] in functions:
                ...
                if i == 0 or toks[i-1] not in type_tokens:
